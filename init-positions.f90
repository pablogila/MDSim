! init-positions.f90  v2023.02.27  by Pablo Gila-Herranz
!
! Based on the programs by Jesús Ugalde and Iñaki Fernández
!
! This program is a simple molecular dynamics simulation of a 3D system of N particles
! in a cubic box of side L. The particles will later interact via a Lennard-Jones potential, and
! we will later use the Verlet algorithm to integrate the equations of motion.
! It also computes the total kinetic energy per particle, the total pressure of the system,
! and the total kinetic, potential, and total energy of the system. 
! The program also computes the histogram of how much atoms are with each velocities in a Delta range.
! It also calculates de histogram for the radial position, the so called radial distribution function.
! This program initialize all the parameters of our system,
! and plots a 3D homogeneous cube as its inicial state.                                                   -
! It also adds the initial velocities, which equals the initial momentum, since we assume m = 1
!
...
!
! Note for continuity purposes: The program was previously called "Initialize-Final.f90"
!
!------------------------------------------------------------
...
real(kind=dp), parameter :: d_zero = L				! L-J min radious
real(kind=dp), parameter :: sigma = d_zero * 0.5_dp ** (1 / 6)
real(kind=dp), parameter :: eps = 1.0_dp
...
Delta = (sqrt(3.0_dp) / 2.0_dp) / nhis
!sqrt(3)/2 is the maximun value of Vsq. So we divide it in nhis small lenght parts
...
!---------------------------------------------------------------------------------
!
! Now we will compute the kinetic energy per particle and the total kinetic energy
!
!---------------------------------------------------------------------------------
...
! ---------------------------------------------------------------------------------
!
! Now we will compute the potential energy of the system energy
!
...
!--------------------------------------------------------------------------------
!
! Now we will generate the radial function of the position (e.g. histogram of the positon)
